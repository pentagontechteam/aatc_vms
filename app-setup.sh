...
# Update .env file for Docker setup (for runtime environment variables)
...
print_status "Building and starting Docker containers with runtime environment variables..."
if docker compose --env-file .env up --build -d; then
    print_success "Docker containers started successfully with runtime environment variables"
else
...
print_status ""
print_status "Environment variables are loaded from .env file at runtime"
print_status ""
...
print_status "  - Restart with new env vars: docker compose down && docker compose --env-file .env up -d"
